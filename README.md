...
[官方网站 / Website](https://floatline.app/) · [发布状态 / Release status](#release-status) · [许可证 / License](#license)
...
官网绑定自定义域名 **[floatline.app](https://floatline.app/)**（GitHub Pages）。

在域名注册商（当前为 GoDaddy）添加以下 DNS 记录，并关闭 GoDaddy 自带建站/停车页：
...
然后在 GitHub 仓库 **Settings → Pages → Custom domain** 填入 `floatline.app`，勾选 **Enforce HTTPS**。DNS 生效后全球即可通过 `https://floatline.app` 访问。
